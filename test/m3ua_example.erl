...
rx_m3ua_prim(#primitive{subsystem = 'M', gen_name = 'SCTP_ESTABLISH', spec_name = confirm}, L) ->
	gen_statem:cast(L#loop_dat.m3ua_pid, osmo_util:make_prim('M','ASP_UP',request));

rx_m3ua_prim(#primitive{subsystem = 'M', gen_name = 'ASP_UP', spec_name = confirm}, L) ->
	gen_statem:cast(L#loop_dat.m3ua_pid, osmo_util:make_prim('M','ASP_ACTIVE',request));
...
tx_sccp_udt(ScrcPid) ->
	...
	gen_statem:cast(ScrcPid, osmo_util:make_prim('N','UNITDATA',request,Opts)).
